...
def register_user():
    ...
    user = User.find_first('where email=?', email)
    if user:
        raise APIError('register:failed', 'email', 'Email is alredy in use.')
    user = User(name=name, email=email, password=password,
                image='https://www.gravatar.com/avatar/%s?d=mm&s=120' % hashlib.md5(email).hexdigest())
    user.insert()
    # make session cookie:
